...
**BUT**, as of writing;
- **memoryview is broken in the current general-availability version of the micropython esp8266 port, hence memory utilisation is sub-optimal (for now) and excessive heap-fragmentation is likely.**
- **contrary to the documentation current general-availability version of the micropython esp8266 port ucollections module does not define an OrderedDict class.**
Therefore **usnmp_esp.py** is a modified version that uses a vanila Dictionary instead.
This results in behaviour that diverges from SNMP standard (wherein Variable Bindings are normally ordered) and as a side-effect you cannot compare a binary packet received from the network with `usnmp.SnmpPacket.tobytes()`, because order in which variable bindings is encoded is unpredictable.
- **usnmp_esp.py is still too large to be loaded by the current general-availability release of micropython esp8266 port**.

__Note: content of v1 subdir is an earlier, much less 'optimal' version maintained for posterity.__
...
convenience alternative accepted by the module;
...
__seeding **.id** this from the likes of micropython's time.ticks_us() may be useful__
...
Indicates SNMP Version 1 (the only version supported by this module).
...
__Output can be unpredictable if **ptr** doesn't point to the first byte
in a data block.  For example, if the byte at ptr; 

- is not an ASN.1 or SNMP type code, an exception will be raised
- is a valid type code (randomly encountered within a payload or length encoding) output may be garbage
__ 
