...
# TODO: learn from webrepl_cli.py

# TODO: use uhashlib refer esp8266/scripts/websocket_helper.py
from http.sha1 import sha1
...
class WebSocket:
    ...
    def __init__(self, request, conn, buflen=128):
        ...
        # TODO: why precalc & send? just send?
        # negotiate websocket, assumes caller tested is_websocket_request
        options = { "Upgrade": b"websocket", "Connection": b"Upgrade" }
        options["Sec-WebSocket-Accept"] = \
            b2a_base64(unhexlify(sha1(
                bytes(request.options["Sec-WebSocket-Key"],"utf-8") + self.MAGIC
        ...
        self.conn.send(b"HTTP/1.1 101 Switching Protocols\r\n")
        for key in options:
            self.conn.send(
                bytes(key, "utf-8") + b":" \
                + options[key] + b"\r\n" # options values are bytes
            )
        self.conn.send(b"\r\n")
